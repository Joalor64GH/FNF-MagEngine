...
class ModDownloadState extends MusicBeatState
{
	...
	public static function downloadInputtedMod()
	{
		...
		var websiteJSON = new haxe.Http("https://gamebanana.com/apiv8/Mod/365489/DownloadPage");
		...
		websiteJSON.onData = function(swagDat:String)
		{
			...
			if (FileSystem.exists('mods/' + coolParsed._aFiles[0]._sFile))
			{
			...
			modDownload.addEventListener(Event.COMPLETE, function(completed)
			{
				...
				for (file in FileSystem.readDirectory('mods/'))
				{
					if (file.endsWith('.zip'))
					{
						...
						coolText.text = 'Finished Downloading ${coolParsed._aFiles[0]._sFile}! Located In "mods/${coolParsed._aFiles[0]._sFile}"';
					}
				}
			});
